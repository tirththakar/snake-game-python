...
def ate_food(x_pos, y_pos, food_x, food_y):
    if x_pos == food_x and y_pos == food_y:
        food_x = round(random.randrange(0, interface_width - snake_dimension))
        food_y = round(random.randrange(0, interface_height - snake_dimension))
    return food_x, food_y

def display_message(message, colour):
    msg = pygame.font.SysFont(None, 50).render(message, True, colour)
    game_interface.blit(msg, [interface_width/3, interface_height/3])
